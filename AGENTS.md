...
15. **Retry only GitLab infrastructure failures** — Automatic CI retries must never include `script_failure`, because a later passing attempt can hide a real test or build failure. Parent-pipeline jobs should extend `.retry_on_infrastructure_failure`; child-pipeline jobs should use the same structured `retry:` policy. Manual retries are unaffected.
